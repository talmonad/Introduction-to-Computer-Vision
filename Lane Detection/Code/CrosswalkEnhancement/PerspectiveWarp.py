import numpy as np
import cv2
from ImageWarping import warp_points
...
class PerspectiveWarp:
    def __init__(
        self,
        dst_size=(1920, 1080),
        src=np.float32([(0.45, 0.6), (0.55, 0.6), (0.2, 1.0), (0.6, 1.0)]),
    ...
    def perspective_warp(self, img):
        ...
        if img is None or len(img.shape) < 2:
            raise ValueError(
                "Invalid input image. Make sure it's a valid 2D or 3D array."
            )
        ...
        # src_scaled, dst_scaled = warp_points(img.shape[:2])
    ...
    def set_dst_points(self, new_dst):
        ...
        if len(new_dst) != 4:
            raise ValueError(
                "Destination points must contain exactly 4 points."
            )

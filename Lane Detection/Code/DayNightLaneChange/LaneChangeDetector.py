...
class LaneChangeDetector:
    def __init__(self, threshold=100, temporal_window=50, history_limit=10):
        self.threshold = threshold  # Sensitivity threshold for lane change detection
    ...
    def smooth_changes(self, changes, smoothing_window=5):
        ...
        return np.convolve(changes, np.ones(smoothing_window) / smoothing_window, mode='valid')
    ...
    def plot_temporal_hist(self):
        ...
        ax[0].plot(range(len(smoothed_left)), smoothed_left, label="Right Changes")
        ax[0].axhline(self.threshold, color="r", linestyle="--", label="Threshold")
        ax[0].set_title("Temporal Right Lane Changes")
        ax[0].set_ylabel("Change Magnitude")
        ax[0].legend()

        # Plot right lane changes
        ax[1].plot(range(len(smoothed_right)), smoothed_right, label="Left Changes")
        ax[1].axhline(-1*self.threshold, color="r", linestyle="--", label="Threshold")
        ax[1].set_title("Temporal Left Lane Changes")
        ax[1].set_ylabel("Change Magnitude")
        ax[1].legend()

        # Plot lane change events
        ax[2].plot(range(len(self.temporal_lane_change_flags)), self.temporal_lane_change_flags, label="Lane Change Events")
